...
{
    private static double[,] chords = new double[108,12];
    ...
    {
        if (chordsInGame.Contains(index))
        {
            return (1.0, index);
        }
        else
        {
            ...
            for (int i = 0; i < chordsInGame.Length; i++)
            {
                for (int j = 0; j < 12; j++)
                {
                    differences[j] = (int)Math.Abs(chords[chordsInGame[i], j] - chords[index, j]);
                }
                TotalDifferences[i] = differences.Sum();
            }
            int minValue = TotalDifferences.Min();
            int minIndex = chordsInGame[Array.IndexOf(TotalDifferences, minValue)];
            double multiplier = GetMultiplier(minValue);
